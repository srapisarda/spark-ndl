...
/**
  ...
  */
class TestMyJoin3 extends FunSpec {
  ...
  describe("ndl rewriting") {
    it("should run the  following tests") {
      ...
      sc.setCheckpointDir("/Users/salvo/Development/uni/spark-ndl/src/test")
      ...
      val a = sc.textFile("src/resources/data/20mb-a.txt")

      val b = sc.textFile("src/resources/data/20mb-b.txt")

      val r = sc.textFile("src/resources/data/20mb-r.txt").map(line => (line.split(',')(0), line.split(',')(1)))

      val s = sc.textFile("src/resources/data/20mb-s.txt").map(line => (line.split(',')(0), line.split(',')(1)))
